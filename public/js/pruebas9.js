 ...
 * NUEVO v3.6.2
 *
 *   - Trazabilidad automática de configuración de pesos por predicción.
 *   - Persistencia solicitada de origen, optimización, evolución y pesos.
 *   - Auditoría de trazabilidad independiente de los pesos activos actuales.
 *   - Compatibilidad con predicciones históricas anteriores a v3.6.2.
 *
 * NUEVO v3.5.3
 *
...
class EntornoPruebas {
    ...
    /*================================================================
        TRAZABILIDAD AUTOMÁTICA DE PREDICCIÓN
        v3.6.2
    ================================================================*/

    async construirTrazabilidadPrediccion() {
        ...
        if (
            this.configuracionPesosService &&
            typeof this.configuracionPesosService
                .obtenerConfiguracionActiva ===
                "function"
        ) {
    ...
    async enriquecerPrediccionConTrazabilidad(
        prediccion
    ) {
        ...
        if (
            ...
            throw new Error(
                "No existe una predicción válida para trazar."
            );
        ...
        /*
         * Si una predicción ya posee trazabilidad, no la reemplazamos.
         * Esto preserva la configuración histórica que realmente la
         * produjo, incluso si los pesos activos cambian más adelante.
         */
        if (
            prediccion.trazabilidad &&
            typeof prediccion.trazabilidad ===
                "object"
        ) {
        ...
        const trazabilidad =
            await this
                .construirTrazabilidadPrediccion();
    ...
    async guardarPrediccion(
        ...
        const objetivoTrazado =
            await this
                .enriquecerPrediccionConTrazabilidad(
                    objetivo
                );
...
async prepararYGuardarPrediccionSegura(
    ...
    async resolverOptimizacionAdaptativa(
        ...
        if (
            ...
            throw new Error(
                "No existe una optimización disponible para el ciclo adaptativo."
            );
        ...
        /*
         * Siempre recargamos desde Firestore para evitar trabajar
         * con objetos viejos cuyo estado haya cambiado en memoria.
         */
        ...
        if (
            ...
            throw new Error(
                `No fue posible recuperar la optimización ${objetivo.id}.`
            );
    ...
    async auditarOptimizacionAdaptativa(
        optimizacion = null
    ) {
        ...
        const objetivo =
            await this
                .resolverOptimizacionAdaptativa(
                    optimizacion
                );


        const pesosActuales =
            objetivo.pesosActuales ||
            {};

        const pesosPropuestos =
            objetivo.pesosPropuestos ||
            {};
        ...
        const auditoria = {
            ...
            evolucionId:
                objetivo.evolucionId ??
                null,

            evaluaciones:
                objetivo.cantidadEvaluaciones ??
                null,

            minimoEvaluaciones:
                objetivo.minimoEvaluaciones ??
                null,

            datosSuficientes:
                objetivo.datosSuficientes ===
                true,

            normalizacionAplicada:
                objetivo.normalizacionAplicada ===
                true,

            sumaActual:
                objetivo.sumaPesosActuales ??
                this.configuracionPesosService
                    .sumarPesos(
                        pesosActuales
                    ),
            ...
            motoresAumentados:
                objetivo.motoresAumentados ||
                [],

            motoresReducidos:
                objetivo.motoresReducidos ||
                [],

            motoresSinCambio:
                objetivo.motoresSinCambio ||
                [],

            advertencias:
                objetivo.advertencias ||
                [],

            rankingSimulacion:
                objetivo.rankingSimulacion ||
                [],
    ...
    async aprobarOptimizacionAdaptativa(
        optimizacion = null,
        motivo =
            "Aprobación controlada de optimización adaptativa."
    ) {
        ...
        if (
            ...
            throw new Error(
                `La optimización ${objetivo.id} no puede aprobarse desde el estado ${objetivo.estado}.`
            );
    ...
    async aplicarOptimizacionAdaptativa(
        optimizacion = null,
        {
            descripcion =
                "Aplicación de pesos desde optimización adaptativa.",
            motivo =
                "Optimización adaptativa validada y aplicada.",
            motivoEstado =
                "Optimización marcada como aplicada después de actualizar la configuración de pesos."
        } = {}
    ) {
        ...
        if (
            ...
            throw new Error(
                `La optimización debe estar APROBADA o APLICADA. Estado actual: ${objetivo.estado}`
            );
        ...
        const configuracionAntes =
            await this.configuracionPesosService
                .obtenerConfiguracionActiva();


        const pesosYaActivos =
            configuracionAntes &&
            configuracionAntes.optimizacionId ===
                objetivo.id &&
            this.configuracionPesosService
                .pesosIguales(
                    configuracionAntes.pesos,
                    objetivo.pesosPropuestos
                );
        ...
        /*
         * Si todavía estaba APROBADA, recién después de haber
         * persistido la configuración la marcamos APLICADA.
         */
        ...
        if (
            ...
            throw new Error(
                "La optimización fue aplicada pero MotorManager no quedó sincronizado."
            );
        ...
        if (
            !coincideConPropuesta
        ) {

            throw new Error(
                "Los pesos activos no coinciden con la propuesta de optimización aplicada."
            );
        ...
        const resultado = {
            ...
            origen:
                configuracionFinal.origen,

            sumaPesos:
                configuracionFinal.sumaPesos,
    ...
    async generarPrediccionPostOptimizacion({
        ...
        if (
            ...
            throw new Error(
                "No se puede generar la predicción: MotorManager no está sincronizado con los pesos activos."
            );
        ...
        if (
            ...
            throw new Error(
                "No existe una configuración activa de pesos."
            );
        ...
        if (
            ...
            return {
                ...
                motivo:
                    exigirPersistida ===
                    true
                        ? "La predicción no contiene trazabilidad persistida."
                        : "Predicción previa a v3.6.2: trazabilidad persistida no disponible."
